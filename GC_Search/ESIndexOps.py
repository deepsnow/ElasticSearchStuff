...
class ESIndexOps:
    ...
    def __init__(self):
        ...
        self.index_name = 'gc_index'
        

    def CreateGCIndexWithMapping(self):
        index_json = json.dumps(self.index_config)
        self.es.create(index=self.index_name, body=index_json)
...
class ESIndexOpsTests(unittest.TestCase):
    ...
    def test_CreateGCIndexWithMapping_ESCalledAppropriately(self):
        with patch.object(Elasticsearch, 'create', return_value=None) as mock_method:
            self.esio.CreateGCIndexWithMapping()
        mock_method.assert_called_once_with(index=self.esio.index_name, body=json.dumps(self.esio.mapping))
